...
def load_data(url):
    ...
    df.columns = df.columns.str.strip().str.replace('\u200b', '')  # remove espaços e zero-width spaces
    # Renomeia para uso interno
...
st.dataframe(ranking_sorted[["UF", base_year, ano, "Perda Absoluta", "Variação (%)"]].head(10))
...
A projeção é feita usando **Facebook Prophet**, que considera:
